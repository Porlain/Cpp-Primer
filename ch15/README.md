...
```c++
// The print function in the derived class will recursively call the current print until memory is exhausted.
class base {
public:
...
```c++
// answer 
(a) Call print in base during compilation.
(b) Call print in derived during compilation.
(c) Call the name function in base, which is not a virtual function and has no dynamic binding, so it is determined at compile time.
(d) Call the name function in derived, which is not a virtual function and has no dynamic binding, so it is determined at compile time.
(e) Call the print function in base, which is determined at runtime.
(f) Call the print function in derived, which is determined at runtime.
```
...
> Write code to verify whether your answers to the previous two questions are correct.
...
[main](ex15.23/ex15.23.cpp) | [Disscussion on SO](http://stackoverflow.com/questions/21320779/trying-to-understand-dynamic-binding-and-virtual-functions)
